...
import { useAutoSave, type SavedWorkflowData } from "../hooks/useAutoSave";
...
export const WorkflowEditor: React.FC = () => {
    ...
    >(new Map());
    const [workflowValidationErrors, setWorkflowValidationErrors] = useState<ValidationError[]>([]);

    // restore dialog state
    const [showRestoreDialog, setShowRestoreDialog] = useState(false);
    const [savedWorkflowData, setSavedWorkflowData] = useState<SavedWorkflowData | null>(null);

    // checks if workflow is valid
    const isWorkflowValid = useMemo(
        () => nodeValidationErrors.size === 0 && workflowValidationErrors.length === 0,
        [nodeValidationErrors, workflowValidationErrors]
    );

    // import useAutoSave hook
    const { saveStatus, lastSavedAt, restoreSavedWorkflow, clearSavedWorkflow } = useAutoSave(
        nodes,
        edges,
        isWorkflowValid
    );
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            ...
            // validate each node
            nodes.forEach((node) => {
                const result = validateNode(node.type || "", node.data);
                // if node is not valid, add errors to map
                if (!result.isValid) {
                    const errorsWithNodeId = result.errors.map((err) => ({
                        ...err,
                        nodeId: node.id,
                    }));
                    nodeErrors.set(node.id, errorsWithNodeId);
                }
            });

            // set node validation errors
            setNodeValidationErrors(nodeErrors);

            // validate workflow structure
            const workflowResult = validateWorkflow(nodes, edges);
            setWorkflowValidationErrors(workflowResult.errors);
        }, 300);
    ...
    useEffect(() => {
        const saved = restoreSavedWorkflow();
    ...
    // combine workflow & node validation errors
    const allValidationErrors: ValidationError[] = useMemo(
        () => [...workflowValidationErrors, ...Array.from(nodeValidationErrors.values()).flat()],
        [workflowValidationErrors, nodeValidationErrors]
    );
    ...
    const handleRestoreWorkflow = () => {
        if (savedWorkflowData) {
            // Restore nodes with onDelete callbacks
            const restoredNodes = savedWorkflowData.nodes.map((node) => ({
                ...node,
                data: {
                    ...node.data,
    ...
    return (
        <Flex minHeight="100vh" direction="column" style={{ width: "100%", position: "relative" }}>
            <Card m="4" mb="0">
            ...
            <Flex flexGrow="1" m="4" mt="2" gap="4">
                ...
                <Flex direction="column" gap="4">
                    ...
                    <ValidationPanel errors={allValidationErrors} />
                </Flex>
                ...
                {selectedNode && (
                    <NodeEditor
                        ...
                        onClose={closeEditor}
                        onDelete={deleteNode}
                        validationErrors={nodeValidationErrors.get(selectedNode.id) || []}
                    />
                )}
            </Flex>
            ...
            <AlertDialog.Root open={showSaveDialog} onOpenChange={setShowSaveDialog}>
                <AlertDialog.Content maxWidth="450px">
                    ...
                    <AlertDialog.Description size="2">
                        Your workflow configuration has been saved to the browser console. Check the
                        developer console for the complete configuration details.
            ...
            <AlertDialog.Root open={showRestoreDialog} onOpenChange={setShowRestoreDialog}>
                <AlertDialog.Content maxWidth="450px">
                    ...
                    <AlertDialog.Description size="2">
                        Found a saved workflow from{" "}
                        {savedWorkflowData?.timestamp
                            ? new Date(savedWorkflowData.timestamp).toLocaleString()
                            : "earlier"}
...
export const NodeEditor: React.FC<NodeEditorProps> = ({
    ...
    const addField = () => {
        ...
        const newFields = [...((formData as FormNodeData).fields || []), newField];
                        ...
                        {((formData as ConditionalNodeData).routes || []).map((route) => (
                            <Card
                                ...
                                style={{
                                    padding: "12px",
                                    backgroundColor:
                                        route.id === "true" ? "var(--green-3)" : "var(--red-3)",
                                }}
                            >
                                <Flex direction="column" gap="2">
                                    <Flex justify="between" align="center">
                                        ...
                                        <TextField.Root
                                            ...
                                            onChange={(e) => {
                                                const conditionalData =
                                                    formData as ConditionalNodeData;
                                                const newRoutes = (
                                                    conditionalData.routes || []
                                                ).map((r) =>
                                                    r.id === route.id
